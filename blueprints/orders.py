...
def create_order():
    ...
    #! NECE DA RADI QUERY
    cursor.execute("INSERT INTO order(order_price, order_date, order_status, user_user_id, adress_adress_id, adress_city_city_id) VALUES(%(order_price)s, %(order_date)s, %(order_status)s, %(user_id)s, %(adress_street)s, %(city_name)s)", response)
    db.commit()
